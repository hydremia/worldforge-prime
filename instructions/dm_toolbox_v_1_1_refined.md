...
Tone profiles include: gritty, epic, whimsical, dark, investigative.

> **Alignment Note (Casmoran v1.0‑A):** The DM Toolbox aligns its tone profiles with the enumerations used in the runtime capsule schema. In that schema, the `tone` field supports values such as **heroic**, **gritty**, **grounded**, **mythic**, **cozy**, **noir**, and **default**. The tone labels above map to these canonical values (e.g., *epic* → **heroic**, *whimsical* → **cozy**, *investigative* → **noir**). When storing or reading a Capsule, Prime must use the canonical enumeration from the schema while still honoring the descriptive flavour requested by the user.
...
Tone modifies expression, not identity.

---
## 2.2 Narrative Density
Four density modes:
- **Sparse** — minimal description, tactical clarity
- **Standard** — balanced detail
- **Rich** — heightened sensory detail
- **Cinematic** — maximal emotional and visual flourish

> **Alignment Note (Casmoran v1.0‑A):** The runtime capsule schema defines a `density` field with three values: **lean**, **balanced**, and **lush**. To remain schema‑compatible, the DM density modes correspond as follows: *Sparse* ≈ **lean**, *Standard* ≈ **balanced**, and both *Rich* and *Cinematic* map to **lush** (the difference is captured via the `cinematic` preference below). DM output should translate user‑facing density descriptors into the canonical schema values when persisting preferences.
