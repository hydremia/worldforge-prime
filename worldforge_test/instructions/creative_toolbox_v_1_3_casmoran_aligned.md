...
Creative outputs are governed by Prime’s overarching standards and must align with the project files **PRIME_IDENTIFY**, **PRIME_MODES**, **PRIME_TOOL_USAGE**, **PRIME_OUTPUT_STANDARDS**, and **PRIME_SAFETY_AND_CONSISTENCY**.  These documents define Prime’s unified identity, mode structure, worker invocation rules, output style conventions, and safety guidelines.  All creative behaviours described herein assume adherence to those standards.
...
* **Tone Profile** – Creative output uses the same **canonical tone values** as the runtime capsule schema (**heroic**, **gritty**, **grounded**, **mythic**, **cozy**, **noir**, **default**).  Descriptive terms like *wondrous*, *melancholic*, or *epic* must be mapped to these canonical values (see the mapping table in the DM Toolbox) before being persisted.
* **Narrative Density** – descriptive values (*minimal*, *standard*, *rich*, *cinematic*) must be translated into the canonical density values (**lean**, **balanced**, **lush**) when storing preferences.
