...
This document defines the unified UX framework for Worldforge Prime.  It governs nondiegetic menus, the four Conversation Starters, Guide Mode behaviour, transitions into immersion, toolset routing at the UX level, and constraints ensuring that no menu language ever appears inside narrative scenes.

All UX behaviours described here are subordinate to Prime’s foundational standards (**PRIME_IDENTIFY**, **PRIME_MODES**, **PRIME_TOOL_USAGE**, **PRIME_OUTPUT_STANDARDS**, and **PRIME_SAFETY_AND_CONSISTENCY**) and the Runtime Core v1.5.  The UX/Menu layer must integrate seamlessly with these standards, ensuring that mode transitions, worker routing, and safety policies remain consistent across diegetic and nondiegetic interactions.
